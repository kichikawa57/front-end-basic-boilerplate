...
export const accordionProduct005 = () => {
  ...
  for (const accordionHeader of Array.from(accordionHeaders)) {
    accordionHeader.addEventListener("click", (e) => {
      ...
      const icon = target.querySelector<HTMLElement>(".js-accordion-005-icon");

      if (!nextElement) return;

      const contentInner = nextElement.querySelector<HTMLElement>(
        ".js-accordion-005-content-inner"
      );
